...
protocol MainFuelListLayoutViewLogic: class {
	func savingsButtonPressed()
	func accuracyButtonPressed()
}

protocol MainFuelListLayoutViewDataLogic: class {
	func updateData(data: [[MainFuelList.FetchPrices.ViewModel.DisplayedPrice]])
}

class MainFuelListLayoutView: UIView, UITableViewDataSource, UITableViewDelegate, MainFuelListLayoutViewDataLogic {

	weak var controller: MainFuelListLayoutViewLogic? 
	...
	func setup() {
		Bundle.main.loadNibNamed("MainFuelListLayoutView", owner: self, options: nil)
		addSubview(baseView)
	...
	// MARK: MainFuelListLayoutViewDataLogic
